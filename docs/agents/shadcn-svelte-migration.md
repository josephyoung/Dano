...
| Dialog | `ui/dialog` | `ThemeSettingsDialog`, `ExtensionDialog`, `ImageLightbox`, `ui/command-dialog` | Official modal lifecycle, focus, Escape, scroll lock, Portal and Overlay. `Content` accepts `overlayProps` for established backdrops and localizes its default close control; covered by the three feature tests and rendered modal flow. |
...
| Tooltip | `ui/tooltip` | `ChatTranscript`, `SubmittedAnswerValue`, `QuestionFieldLabel` | Official provider/root/trigger/content layer and `.app-shell` portal. `--layer-tooltip: 1000` preserves the pre-migration value without defining #352's final layer ordering; `themes/index.test.ts` guards the token. |
...
The tooltip wrapper uses `--layer-tooltip: 1000`, exactly matching its pre-migration explicit value. It intentionally shares the current dialog-overlay value; defining a new global ordering here would pre-empt #352. That follow-up owns any category or ordering change after reproducing cross-feature concurrency against this converged baseline.
