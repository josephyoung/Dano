...
| Alert Dialog | `ui/alert-dialog` | `ReauthenticationDialog` | Official modal lifecycle, focus handling and shared `.app-shell` portal. Feature state keeps the required reauthentication choice open until the user explicitly continues anonymously or starts login again; covered by `ReauthenticationDialog.test.ts`. Extension confirmation stays in its single protocol-request Dialog lifecycle rather than splitting one request across modal roots. |
| Alert | `ui/alert` | `ReauthenticationDialog` | Official semantic feedback surface. The destructive variant reports a failed transition to Anonymous without dismissing the required choice; `ReauthenticationDialog.test.ts` verifies its `role="alert"` projection. |
...
| `ReauthenticationDialog`, called by `App` | Own the required choice after a Login Session can no longer obtain a valid credential: continue as Anonymous or start login again | Shared `AlertDialog` owns modal, focus and Portal behavior; shared `Alert` reports a failed Anonymous transition. Feature state keeps the choice visible until an explicit branch succeeds | `ReauthenticationDialog.test.ts` verifies the alert-dialog, both actions, failure alert and Escape behavior; `bridgeStore.prompt.test.ts` and `oauth-login-http.test.ts` cover the server projection and Anonymous fallback. The in-app Browser credential-expiry flow verifies the prompt, login restart and Anonymous return against the rendered app |
