...
| Dialog | `ui/dialog` | `ThemeSettingsDialog`, `ExtensionDialog`, `ImageLightbox`, `ui/command-dialog` | Official modal lifecycle, focus, Escape, scroll lock, Portal and Overlay. `Content` accepts `overlayProps` for established backdrops, a semantic `dialog`/`lightbox` layer, and localizes its default close control; covered by the three feature tests and rendered modal flow. |
...
1. Popovers, selects, and date pickers
2. Dialog/sheet/alert-dialog overlays and content
3. Tooltips
4. Image-lightbox overlay and content
5. Notifications
6. Native `<dialog>.showModal()` top layer, which the browser places above all ordinary stacking contexts

Shared wrappers apply these defaults. Feature components may select the named `lightbox` Dialog layer when that semantic is intrinsic to the feature, but must not assign cross-feature numeric `z-index` values. Small values used inside an isolated feature surface remain implementation detail rather than part of this ordering.
