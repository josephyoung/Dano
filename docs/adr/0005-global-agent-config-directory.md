...
Existing files in the Agent Config Directory are host-managed persistent runtime configuration. Application and ordinary container startup must not overwrite them. An operator can edit `SYSTEM.md` in the host persistence location. A Release Build is the explicit synchronization boundary: it atomically renders and replaces `SYSTEM.md` once before starting the new app image, while later restarts preserve that deployed or host-edited content.
