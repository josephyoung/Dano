...
Use `scripts/deploy-env-file.mjs` for allowlisted `.env` changes. Updating a key must fold all duplicates to one entry; an empty optional value removes the key instead of writing `''`. Never print before/after env contents or values. Before switching, run `node scripts/check-oauth-relay-contract.mjs`; OAuth endpoints under `/admin-api/` require an explicit `DANO_OAUTH_RELAY_ORIGIN`, the upstream path must preserve `/admin-api/`, and `/assets/`, logo, and favicon references must remain under `/admin-api/` without a hardcoded provider domain. The checker must validate nginx-parsed rendered configuration so comments cannot satisfy the contract.
