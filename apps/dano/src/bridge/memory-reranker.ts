...
export class MemoryReranker {
  ...
  async filter(query: string, candidates: RecalledMemory[], signal?: AbortSignal): Promise<RecalledMemory[]> {
    ...
    const { url, model, minimumLogit, timeoutMs, maxInputBytes, maxDocumentBytes } = this.#config;
    if (candidates.some(item => Buffer.byteLength(item.text) > maxDocumentBytes)
      || Buffer.byteLength(query) + Buffer.byteLength(JSON.stringify(candidates.map(item => item.text))) > maxInputBytes) return [];
    try {
      const response = await fetch(url, { method: "POST", redirect: "error",
        headers: { "Content-Type": "application/json" },
        signal: AbortSignal.any([AbortSignal.timeout(timeoutMs), ...(signal ? [signal] : [])]),
        body: JSON.stringify({ model, query, documents: candidates.map(item => item.text), top_n: candidates.length }) });
      if (!response.ok) return [];
      const payload: unknown = await response.json();
      if (!payload || typeof payload !== "object" || Array.isArray(payload)) return [];
      const results = (payload as Record<string, unknown>).results;
      if (!Array.isArray(results) || results.length !== candidates.length) return [];
      ...
      for (const result of results) {
        ...
        if (!Number.isSafeInteger(index) || (index as number) < 0 || (index as number) >= candidates.length
          || seen.has(index as number) || typeof logit !== "number" || !Number.isFinite(logit)) return [];
        seen.add(index as number);
        if (logit >= minimumLogit) ranked.push({ item: candidates[index as number]!, logit });
      }
